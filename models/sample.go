...
func SearchSamplesWithPagination(name string, page, limit int) ([]Sample, error) {
	...
	defer db.Close()
	if result := orm.Where("name LIKE ?", "%"+name+"%").Order("updated_at desc").Offset(page * limit).Limit(limit).Find(&samples); result.Error != nil {
...
func GetLastSampleResultBySampleID(sampleID string) (*SampleResult, error) {
	...
	if result := orm.Where("sample_id = ?", sampleID).Order("created_at desc").First(&sampleResult); result.Error != nil {
...
func SearchSamples(name string) ([]Sample, error) {
	...
	defer db.Close()
	if result := orm.Order("updated_at desc").Where("name LIKE ?", "%"+name+"%").Find(&samples); result.Error != nil {
		return nil, result.Error
	}
	return samples, nil
}
...
func GetSamplesWithPagination(page, limit int) ([]Sample, error) {
	...
	defer db.Close()
	if result := orm.Order("updated_at desc").Offset(page * limit).Limit(limit).Find(&samples); result.Error != nil {
		return nil, result.Error
	}
	return samples, nil
}
...
func GetSamples() ([]Sample, error) {
	...
	if result := orm.Order("updated_at desc").Find(&samples); result.Error != nil {
		return nil, result.Error
	}
...
func GetSampleResults(sampleID string, limit int) ([]SampleResult, error) {
	...
	if result := orm.Where("sample_id = ?", sampleID).Order("created_at desc").Limit(limit).Preload("Agent").Find(&sampleResults); result.Error != nil {
		return nil, result.Error
	}
...
func GetSampleByID(id string) (*Sample, error) {
	...
	if result := orm.First(&sample, "id = ?", id); result.Error != nil {
		return nil, result.Error
	}
	return &sample, nil
}
...
func GetMetricsBySampleID(sampleID string) ([]Metric, error) {
	...
	if result := orm.Where("sample_id = ?", sampleID).Find(&metricResult); result.Error != nil {
		return nil, result.Error
	}
...
func GetDistinctMetricNameBySampleID(sampleID string) ([]string, error) {
	...
	if result := orm.Model(&Metric{}).Distinct().Where("sample_id = ?", sampleID).Pluck("name", &results); result.Error != nil {
		return nil, result.Error
	}
...
func GetSampleResultBySampleIDWithLimit(sampleID string, limit int) ([]*SampleResult, error) {
	...
	if result := orm.Where("sample_id = ?", sampleID).Order("created_at desc").Limit(limit).Find(&sampleResult); result.Error != nil {
		return nil, result.Error
	}
	return sampleResult, nil
}
...
func GetMetricsBySampleResultID(sampleResultID string) ([]Metric, error) {
	...
	if result := orm.Where("sample_result_id = ?", sampleResultID).Find(&metricResult); result.Error != nil {
		return nil, result.Error
	}
	return metricResult, nil
}
...
func (s *Sample) PushMetrics(ScenarioResult *ScenarioResult, agentID string) error {
	...
	if result := orm.Create(&sampleResult); result.Error != nil {
		return result.Error
	}
...
func UpdateSample(id, name, descrption, kind string, sampleData []byte, user *User) (*Sample, error) {
	...
	// Save the sample
	if result := orm.Save(&updateSample); result.Error != nil {
		return nil, result.Error
	}
...
func RegisterSample(name, descrption, kind string, sampleData []byte, user *User) (*Sample, error) {
	...
	if result := orm.Create(&newSample); result.Error != nil {
		return nil, result.Error
	}
